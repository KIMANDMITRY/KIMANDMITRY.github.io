...
if(document.querySelector('html').offsetWidth < 700){
    $('.road-map__list').slick({
        slidesToShow: 1,
        slidesToScroll: 1,
        autoplay: true,
        autoplaySpeed: 4000,
...
var countDown = new Date('Feb 11, 2018 00:00:00').getTime(),
...
function radialProgress(selector) {
    ...
    var svg = parent.append('svg').attr('width', size.width).attr('height', size.height);
    var outerRadius = Math.min(size.width, size.height) * 0.39;
    var thickness = 2;
    var value = 0;

    var mainArc = d3.arc().startAngle(0).endAngle(Math.PI * 2).innerRadius(outerRadius - thickness).outerRadius(outerRadius);

    svg.append("path").attr('class', 'progress-bar-bg').attr('transform', 'translate(' + size.width / 2 + ',' + size.height / 2 + ')').attr('d', mainArc());

    var mainArcPath = svg.append("path").attr('class', 'progress-bar').attr('transform', 'translate(' + size.width / 2 + ',' + size.height / 2 + ')');

    svg.append("circle").attr('class', 'progress-bar').attr('transform', 'translate(' + size.width / 2 + ',' + (size.height / 2 - outerRadius + thickness / 2) + ')').attr('width', thickness).attr('height', thickness).attr('r', thickness / 2);

    var end = svg.append("circle").attr('class', 'progress-bar').attr('transform', 'translate(' + size.width / 2 + ',' + (size.height / 2 - outerRadius + thickness / 2) + ')').attr('width', thickness).attr('height', thickness).attr('r', thickness / 2);

    var percentLabel = svg.append("text").attr('class', 'progress-label').attr('transform', 'translate(' + size.width / 2.1 + ',' + size.height / 2 + ')').text('0');

    return {
        update: function update(progressPercent) {
            ...
            mainArcPath.transition().duration(transitionDuration).attrTween('d', function () {
                ...
                return function (t) {
                    return 'translate(' + size.width / 2 + ',' + size.height / 2 + ')' + ('rotate(' + (startAngleDeg + angleDiffDeg * t) + ')') + ('translate(0,-' + (outerRadius - thickness / 2) + ')');
                };
            });
...
var chart = radialProgress('.progress-widjet');
var progress = [75];
var state = 0;
